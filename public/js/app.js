/* ══════════════════════════════════════════════════════════════════
   PokéVault app.js  v25
   Requires: Supabase JS v2, Chart.js v4, loaded in index.html

   v25 fixes:
   • saveShareSettings: upsert → update to avoid NOT NULL violation
     on username/is_premium when the profiles row already exists
   • pfSearch / refreshPortfolioValues: pass includeEbay=true so
     graded card eBay pricing data is fetched from the upstream API
   • pokeprice.js: includeEbay added to ALLOWED_UPSTREAM_PARAMS and
     forwarded through safeParams to the upstream /cards endpoint
══════════════════════════════════════════════════════════════════ */
...
async function initAuth() {
  ...
  // Check premium
  try {
    const { data: profile } = await _sb.from('profiles').select('is_premium').eq('id', _user.id).maybeSingle();
...
// Extracts the correct graded price from a PokéPrice result based on the
// condition_or_grade string (e.g. "PSA 10", "CGC 9.5", "BGS 10").
// The API returns graded prices when includeEbay=true. Field names follow
// the pattern: psa10, psa9, cgc10, cgc95, bgs10, sgc10, etc.
// Falls back to raw extractPrice() if no graded price found.
function extractPriceForGrade(r, conditionOrGrade) {
  if (!r || !conditionOrGrade) return extractPrice(r);

  // Normalise: "PSA 10" → "psa10", "CGC 9.5" → "cgc95", "BGS 10" → "bgs10"
  const key = conditionOrGrade
    .toLowerCase()
    .replace(/\s+/g, '')   // remove spaces
    .replace(/\./g, '');   // remove decimals  e.g. 9.5 → 95

  // Recursive deep scan — walks the full result object looking for a field
  // whose name contains the normalised grade key and has a positive numeric value.
  function deepScan(obj, depth = 0) {
    ...
    for (const [k, v] of Object.entries(obj)) {
      ...
      if (typeof v === 'object' && v !== null) {
        const found = deepScan(v, depth + 1);
  ...
  return deepScan(r) ?? extractPrice(r);
}
...
async function savePortfolioItem() {
  ...
  // card's current market price. The price comes directly from the search
  // result object `r` that the user just tapped — no extra API call needed.
  // Fallback: purchase price if market price is unavailable.
  const newId = insertedRows?.[0]?.id;
  if (newId) {
    try {
      const priceUsd     = condition && ['PSA','CGC','BGS','SGC'].some(g => condition.toUpperCase().startsWith(g))
        ? extractPriceForGrade(r, condition)
        : extractPrice(r);
...
/* ── Card detail modal — chart + grade toggle ────────────────────── */
...
let _cdmChart     = null;
let _cdmGradeMode = 'raw';

// Called by openCardDetailModal in index.html to reset grade state
window._resetCdmGrade = function () { _cdmGradeMode = 'raw'; };

async function loadCdmChart(portfolioItemId, mode, cardName, cardSet) {
  const loading = document.getElementById('cdm-chart-loading');
  const canvas  = document.getElementById('cdm-chart-canvas');
  const empty   = document.getElementById('cdm-chart-empty');
  if (!canvas) return;
  ...
  if (_cdmChart) { _cdmChart.destroy(); _cdmChart = null; }

  const accent = '#00e5cc';

  // ── Raw (ungraded): read from price_history table ─────────────────
  if (mode === 'raw') {
    try {
      const { data, error } = await _sb
        .from('price_history')
      ...
      const labels = data.map(r => r.recorded_date);
      const values = data.map(r => parseFloat(r.value_sgd));

      if (loading) loading.style.display = 'none';
      canvas.style.display = 'block';

      _cdmChart = new Chart(canvas, {
        type: 'line',
        data: {
          labels,
          datasets: [{
            label: 'Raw (Ungraded) — SGD',
            data: values,
            borderColor: accent,
        ...
        options: _cdmChartOptions(v => `SGD $${v.toFixed(2)}`),
      });
    } catch (err) {
      if (loading) loading.style.display = 'none';
      if (empty) {
        empty.style.display = 'block';
        empty.textContent = err.message === 'no data'
          ? 'No price history yet. Hit "Refresh values" to start recording.'
  ...
  // ── Graded (PSA 10 / PSA 9 / BGS 10): fetch via PokéPrice includeEbay ──
  // v25: routes through /api/pokeprice?includeEbay=true instead of the
  // separate /api/graded endpoint (which required an EBAY_APP_ID we don't have).
  // PokéPrice returns graded sale prices directly when includeEbay=true.
  const GRADE_LABELS = { psa10: 'PSA 10', psa9: 'PSA 9', bgs10: 'BGS 10' };
  const gradeLabel = GRADE_LABELS[mode] || mode;

  if (!cardName) {
    const item = _portfolio.find(i => i.id === portfolioItemId);
    cardName = item?.name    || '';
    cardSet  = item?.set_name || '';
  }
  ...
  try {
    ...
    const fxRate = _fxRate || 1.35;

    // Log full shape of first result so we can verify graded fields in console
    if (results[0]) {
      console.log('[PokéVault v25] pokeprice includeEbay result[0]:', JSON.stringify(results[0], null, 2));
    }

    // Build data points using the same extractPriceForGrade used for portfolio refresh
    const points = results
      .map(r => {
        const usd = extractPriceForGrade(r, gradeLabel); // e.g. "PSA 10"
        if (usd == null) return null;
        const name = r.name || r.cardName || r.productName || '';
        return { label: name, priceSgd: parseFloat((usd * fxRate).toFixed(2)) };
      })
      .filter(Boolean);

    if (!points.length) throw new Error('no graded price data');

    const sgdPrices = points.map(p => p.priceSgd);
    const avg = parseFloat((sgdPrices.reduce((a, b) => a + b, 0) / sgdPrices.length).toFixed(2));
    ...
    _cdmChart = new Chart(canvas, {
      type: 'bar',
      data: {
        labels: points.map((p, i) => p.label || `#${i + 1}`),
        datasets: [{
          label: `${gradeLabel} Market Price — SGD`,
          data:  points.map(p => p.priceSgd),
          backgroundColor: 'rgba(0,229,204,0.25)',
          borderColor: accent,
          borderWidth: 2,
          borderRadius: 4,
        }],
      ...
      options: {
        ..._cdmChartOptions(v => `SGD $${v.toFixed(2)}`),
        plugins: {
          ...
          tooltip: {
            ...
            titleColor: '#e2e6f0',
            bodyColor: accent,
            callbacks: { label: ctx => `SGD $${ctx.parsed.y.toFixed(2)}` },
          },
        },
        scales: {
          x: { ticks: { color: '#4e566b', maxRotation: 30, font: { size: 10 } }, grid: { color: 'rgba(255,255,255,.04)' } },
          y: { ticks: { color: '#4e566b', callback: v => `$${Number(v).toFixed(0)}`, font: { size: 10 } }, grid: { color: 'rgba(255,255,255,.04)' } },
        },
      },
    });
    ...
    if (empty) {
      ...
      empty.textContent   = `${points.length} result${points.length !== 1 ? 's' : ''} · Avg ${gradeLabel}: SGD $${avg.toFixed(2)}`;
    }
    ...
    if (empty) {
      empty.style.display = 'block';
      empty.style.color   = 'var(--text3)';
      if (err.message === 'no graded price data' || err.message === 'no data') {
        empty.textContent = `No ${gradeLabel} pricing data found for "${cardName}" via PokéPrice.`;
      } else {
        empty.textContent = `Could not load graded data: ${err.message}`;
      }
    }
  }
}
window.loadCdmChart = loadCdmChart;

// Shared Chart.js options for both raw and graded charts
function _cdmChartOptions(tooltipFmt) {
  return {
    responsive: true,
    plugins: {
      legend: { display: false },
      tooltip: {
        ...
        titleColor: '#e2e6f0',
        bodyColor: '#00e5cc',
        callbacks: tooltipFmt ? { label: ctx => tooltipFmt(ctx.parsed.y) } : {},
      },
    },
    scales: {
      x: { ticks: { color: '#4e566b', maxTicksLimit: 6, font: { size: 10 } }, grid: { color: 'rgba(255,255,255,.04)' } },
      y: { ticks: { color: '#4e566b', callback: v => `$${Number(v).toFixed(0)}`, font: { size: 10 } }, grid: { color: 'rgba(255,255,255,.04)' } },
    },
  };
}

function switchCdmGrade(mode, portfolioItemId, btn) {
  _cdmGradeMode = mode;
  document.querySelectorAll('.cdm-grade-btn').forEach(b =>
    b.classList.toggle('active', b.dataset.mode === mode)
  );
  // Read card name + set from the clicked button's data attributes
  const cardName = btn?.dataset?.name || '';
  const cardSet  = btn?.dataset?.set  || '';
  loadCdmChart(portfolioItemId, mode, cardName, cardSet);
}
window.switchCdmGrade = switchCdmGrade;
...
async function saveShareSettings() {
  ...
  const rawSlug  = (slugInput ? slugInput.value : '').trim().toLowerCase();
